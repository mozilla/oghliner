    ...
    resolve(ghslug('./').catch(function() {
      ...
      staticFileGlobs.forEach(function(globPattern) {
        glob.sync(globPattern.replace(path.sep, '/')).forEach(function(file) {
          ...
          if (stat.isFile() && stat.size > 2 * 1024 * 1024 && staticFileGlobs.indexOf(file) === -1) {
            gutil.log(chalk.yellow.bold(file + ' is bigger than 2 MiB. Are you sure you want to cache it? To suppress this warning, explicitly include the file in the fileGlobs list.'));
          }
        });
      });
      ...
      importScripts.forEach(function(script) {
        ...
        try {
          ...
          gutil.log(chalk.red.bold(script + ' doesn\'t exist.'));
          throw ex;
        }

        if (!stat.isFile()) {
          gutil.log(chalk.red.bold(script + ' is not a file.'));
