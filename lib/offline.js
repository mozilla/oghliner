...
module.exports = promisify(function(config, callback) {
  ...
  ghslug('./').catch(function() {
    ...
    var stream = gulp.src([__dirname + '/../templates/app/offliner-worker.js'])
  ...
  function getFilesAndHashes(files, stripPrefix) {
    return files
      .map(function (filepath) {
        ...
        return { filepath: filepath.replace(stripPrefix, ''), hash: hash };
      })
      .map(function (pathAndHash) {
        return '  "' + pathAndHash.filepath + '" /* ' + pathAndHash.hash + ' */';
      })
      .join(',\n');
  }
