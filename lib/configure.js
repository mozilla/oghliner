...
module.exports = function() {
  ...
  .then(function(remotes) {
    ...
    return promptly.prompt('Remote [default: ' + defaultRemote + ']: ', { default: defaultRemote });
  })

  .then(function(remote) {
    return ghslug('./', remote)
    .then(function(slug) {
      process.stdout.write('Ok, I\'ll configure Travis to auto-deploy the ' + remote + ' remote (' + slug + ').\n');
      return slug;
    });
  })
  ...
  .then(function() {
    ...
    .then(function(res) {
      // The Travis token is stored in the res.access_token property.
  ...
  .then(function() {
    ...
    .then(function(res) {
      token = res.token;
      stopSpinner(message);
      res.isRecreated && process.stdout.write(chalk.blue.bold(
        '\n' +
  ...
  .then(function(blob) {
    ...
    // Removes previous implementation of "after_success" step
    // from the .travis.yml file
    if (('after_success' in travisYml) || travisYml.after_success === null) {
      var PREV_IMPLEMENTATION = 'echo "travis_fold:end:after_success" && ' +
      '[ "${TRAVIS_PULL_REQUEST}" = "false" ] && [ "${TRAVIS_BRANCH}" = "master" ] && ' +
      'echo "Deploying…" && npx gulp deploy';
      var after_success = travisYml.after_success || [];
      var updated_after_success = []

      if (after_success.some(function (v) { return v.indexOf(PREV_IMPLEMENTATION) !== -1; })) {
        updated_after_success = after_success.filter(function (v) {
          return !v.includes(PREV_IMPLEMENTATION)
        })

        if (updated_after_success.length === 0) {
          delete travisYml.after_success
        } else {
          travisYml.after_success = updated_after_success
        }
      }
    }

    command = 'echo "travis_fold:end:deploy" && ' +
    ...
    if (!('deploy' in travisYml) || travisYml.deploy === null) {
      travisYml.deploy = travisDeployCfg;
    };

    if (
      travisYml.deploy
      && travisYml.deploy.script
      && !travisYml.deploy.script.includes(command)
      || !travisYml.deploy
    ) {
      travisYml.deploy = travisDeployCfg;
    }
  ...
  .catch(function(ex) {
    if (activeSpinnerMessage) {
      stopSpinner(activeSpinnerMessage, true);
    }
