...
// The message currently displayed with a spinner, if any.  Most async
// operations don't need to set this, but occasionally one displays a message
// and then calls a function that displays another message before
// the operation is complete.  In that case, the function needs to stop
// the spinner for the first message before it displays the second message.
var spinnerMessage;
  ...
  function createToken(scopes, note, noteUrl) {
    ...
    .catch(function(err) {
      ...
      if (error.message === 'Validation Failed' && error.errors[0].code === 'already_exists') {
        ...
        spinnerMessage && cli.spinner('  ' + spinnerMessage, true);
        var message = 'You already have that token; deleting it…';
        cli.spinner('  ' + message);
          ...
          if (hook.owner_name === user && hook.name === repo) {
            ...
            .then(function(res) {
              if (res.result) {
                ...
                cli.spinner(chalk.red('✓ ') + message + ' failed!', true);
